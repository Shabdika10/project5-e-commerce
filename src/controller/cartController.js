...
const updateCart = async function (req, res) {
    try {
        ...
        const dataArray = ['cartId', 'productId', 'removeProduct'];

        for (let i = 0; i < dataArray.length; i++) {
            if (!body[dataArray[i]] && body[dataArray[i]] == undefined) {
                return res.status(400).send({ status: false, message: `${dataArray[i]} field is required` });
            }
        }
        if (!isValid(body)) {
